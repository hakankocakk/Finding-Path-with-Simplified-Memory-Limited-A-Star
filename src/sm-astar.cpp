             ...
             int* pOutBuffer, const int nOutBufferSize)
{
	...
	if (nStartX == nTargetX && nStartY == nTargetY) {
		...
		return 0; // Ba�lang�� ve hedef ayn�ysa yol uzunlu�u 0
	}

	int rv = -1; // Yol bulunamad� varsay�m�

	NodeQueue queue; // A��k k�me (Open Set) - de�erlendirilecek d���mler
	WeakNodeSet allNodes; // Olu�turulan t�m d���mleri takip etme
	// Ba�lang�� d���m�n� olu�tur ve kuyru�a ekle
	...
	std::string input; // Ad�m ad�m ilerlemek i�in kullan�c� girdisi

	// Ana arama d�ng�s�: Kuyruk bo� olana veya hedef bulunana kadar devam et
	while(!queue.empty()) {
		std::cout << "------------------------------------------------------------------------" << std::endl;
		// Kuyru�un ba��ndaki (en d���k maliyetli) d���m� al
		PNode curNode(*queue.begin());

		// E�er mevcut d���m hedef ise
		if (curNode->Offset() == target) {
			rv = curNode->GCost(); // Yol uzunlu�unu al
			// Yolu geriye do�ru izleyerek pOutBuffer'a kaydet
			PNode n = curNode;
			int i = curNode->GCost() - 1; // Arabellek indeksi (hedef dahil, ba�lang�� hari�)
			...
			// E�er hedef ba�lang�� de�ilse yolu kaydet
			if (rv > 0) {
				while (n->Parent() && i >= 0) { // Ba�lang�ca veya arabelle�in sonuna kadar git
					pOutBuffer[i--] = n->Offset();
					n = n->Parent(); // Bir �st d���me ge�
				}
			}
			break; // Hedef bulundu, d�ng�y� sonland�r
		}

		PNode nextNode(nullptr); // Geni�letilecek bir sonraki kom�u d���m

		// E�er mevcut d���m geni�letilebilir durumdaysa (maliyet sonsuz de�ilse)
		if (curNode->FCost() < std::numeric_limits<float>::max()) {
            // *** EKLENEN KISIM: Hangi d���m�n geni�letilece�ini yazd�r ***
            std::cout << "En iyi dugum genisletiliyor: ("
                      << map.X(curNode->Offset()) << ", "
                      << map.Y(curNode->Offset()) << ")" << std::endl;
            // ************************************************************
			nextNode = curNode->ExpandNext(map, queue); // D���m�n bir sonraki kom�usunu geni�let
		}
		...
		if (nextNode) {
            ...
            // ************************************************************
             ...
             Paint(map, curNode, startNode); // Yine de durumu g�rselle�tir
             std::cout << "Toplam Dugum: " << allNodes.size() << ", Kuyrukta (Yaprak): " << queue.size() << std::endl;
			 std::cout << "Mevcut Yol Uzunlugu: " << curNode->GCost() << "/" << nOutBufferSize << std::endl;
			 std::cout << "Bellek Kullanimi: " << float(mem_use)/1000 << " kB."
					<< std::endl << std::endl;
